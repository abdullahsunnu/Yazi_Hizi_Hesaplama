...
class YaziHiziHesaplama():
    ...
    def metinDogruluguHesaplama(self):
        ...
        try:
            showinfo("Bilgilendirme",
                     "Yazma için ayırdığınız süre toplamda " + str(round(self.__zamanBilgileri[2], 2)) + " saniyedir.")
            showinfo("Bilgilendirme", "1 saniyede ortalama {} kelime yazabildiniz.".format(
                round(len(yazilanKelimeler) / (self.__zamanBilgileri[2]), 2)))
            showinfo("Bilgilendirme", "Yazının doğruluk oranı yüzde {}".format(
                100 - round(len(yanlisKelimeIndeksi) / len(asilKelimeler) * 100), 2))
        except:
            showerror("Hata", "Bir hata oluştu. Bu yüzden bilgilendirme yapılamıyor.")
        finally:
            showinfo("Bilgilendirme",
                     "Açılacak olan pencerede yazdıklarınız arasından yanlış sırada ve yazımda olanlar ile eksik/fazla olan kısımlar kırmızı renk ile gösterilmektedir.")

        self.sonucPenceresi = Tk()
        self.sonucPenceresi.title("Yazı Sonuç Ekranı")
    ...
    def pencereYap(self):
        ...
        menuAciklamalar.add_command(label="Program Hakkında Açıklama", command=self.programAciklamasi)
    ...
    def basla(self, durum=True):
        try:
            ...
            if durum:  # if'e girerse Baştan Başla tuşuna basıldı demektir.
                ...
                self.gosterilenMetin.insert(END, metin[0].lstrip(
                    " "))  # ilk cümlenin başında olabilecek boşluk karakteri silinir.
